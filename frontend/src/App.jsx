    ...
    <div style={{ padding: '20px', color: 'white', backgroundColor: '#141414', minHeight: '100vh', fontFamily: 'sans-serif', position: 'relative' }}>
      ...
      {toast.show && (
        <div style={{
          ...
          backgroundColor: toast.isError ? '#ff4d4d' : '#2ecc71',
          color: 'white', padding: '15px 25px', borderRadius: '8px',
          fontWeight: 'bold', boxShadow: '0 5px 15px rgba(0,0,0,0.5)',
      ...
      <nav style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '40px', borderBottom: '1px solid #333', paddingBottom: '20px', flexWrap: 'wrap', gap: '20px' }}>
        ...
        <form onSubmit={handleSearch} style={{ display: 'flex', gap: '10px', flexGrow: 1, maxWidth: '400px' }}>
          <input 
            ...
            style={{ width: '100%', padding: '12px', borderRadius: '5px', border: 'none', backgroundColor: '#333', color: 'white', fontSize: '14px' }}
          />
          <button type="submit" style={{ padding: '10px 20px', backgroundColor: '#E50914', color: 'white', border: 'none', borderRadius: '5px', cursor: 'pointer', fontWeight: 'bold', transition: '0.2s' }}>
            Cari
          </button>
        </form>
        ...
        <div style={{ display: 'flex', gap: '15px' }}>
          <button 
            ...
            style={{ padding: '10px 20px', backgroundColor: showWatchlist ? 'transparent' : '#fff', color: showWatchlist ? '#fff' : '#000', border: '1px solid #fff', borderRadius: '5px', cursor: 'pointer', fontWeight: 'bold' }}
          >
            Beranda
          </button>
          <button 
            onClick={() => toggleView(true)}
            style={{ padding: '10px 20px', backgroundColor: showWatchlist ? '#fff' : 'transparent', color: showWatchlist ? '#000' : '#fff', border: '1px solid #fff', borderRadius: '5px', cursor: 'pointer', fontWeight: 'bold' }}
          >
            My Watchlist
          </button>
          <button 
            onClick={handleLogout}
            style={{ padding: '10px 20px', backgroundColor: '#444', color: 'white', border: 'none', borderRadius: '5px', cursor: 'pointer', fontWeight: 'bold', marginLeft: '10px' }}
          >
            Logout
          </button>
        </div>
      ...
      <main>
        ...
        {loading ? (
          <div style={{ textAlign: 'center', marginTop: '50px' }}>
            ...
            <p style={{ color: '#aaa', fontSize: '14px' }}>Memuat data dari server...</p>
          </div>
        ) : (
          <>
            {showWatchlist && watchlist.length === 0 && (
              <p style={{ color: '#aaa', textAlign: 'center', marginTop: '40px' }}>Belum ada film di daftar tontonanmu. Jelajahi Beranda!</p>
            )}
            
            {!showWatchlist && movies.length === 0 && (
              <p style={{ color: '#aaa', textAlign: 'center', marginTop: '40px' }}>Film tidak ditemukan. Coba kata kunci lain.</p>
            )}
                ...
                (movie.poster_path || movie.poster) && (
                  ...
                  >
                    ...
                    <div style={{ padding: '15px', flexGrow: 1, display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
                      <h3 style={{ fontSize: '15px', margin: '0 0 15px 0', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', fontWeight: 'bold' }}>
                        {movie.title}
                      </h3>
                      
                      {showWatchlist ? (
                        <button 
                          onClick={() => removeFromWatchlist(movie.movie_id, movie.title)}
                          style={{ width: '100%', padding: '10px', backgroundColor: '#222', color: '#ff4d4d', border: '1px solid #ff4d4d', borderRadius: '4px', cursor: 'pointer', fontWeight: 'bold', transition: '0.2s' }}
                        >
                          ❌ Hapus List
                        </button>
                      ) : (
                        <button 
                          ...
                          style={{ width: '100%', padding: '10px', backgroundColor: '#E50914', color: 'white', border: 'none', borderRadius: '4px', cursor: 'pointer', fontWeight: 'bold', transition: '0.2s' }}
                        >
                          + Watchlist
                        </button>
                      )}
